# Contributing to Astrodock

Thanks for your interest. Astrodock is an early-stage rebuild — see `OPEN_SOURCE.md` for the
...
npm run migrate -w @astrodock/control-plane
npm run seed    -w @astrodock/control-plane
...
## Tests
```bash
npm test -w @astrodock/schema          # schema validator + env catalog (no DB)
npm test -w @astrodock/control-plane   # unit + integration (needs a live Postgres)
npm test -w @astrodock/cli             # CLI against an in-process control plane (needs Postgres)
```
...
- Platform-managed config uses the reserved `ASTRODOCK_` env prefix. App-declared vars may not.
